...
const REGEX = Object.freeze({
  ...
  'HEADER6': /^\s*######\s+(.*)/i, 
  'BLOCKQUOTE': /^\s*>\s+(.*)/i,
  'SPACEBREAK': /^(\s*)$/i, // TAOTODO: Space break should break a new p
  'LINEBREAK': /^\s*---(.*)\s*$/i // TAOTODO: Breaker should break a new div
})

const REGEX_INLINE = Object.freeze({
  'BOLD': [/(^|^.*\s+)\*\*(.+)\*\*(\s+.*$|$)/i, /(^|^.*\s+)\<b\>(.+)\<\/b\>(\s+.*$|$)/i],
  'ITALIC': [/(^|^.*\s+)\*(.+)\*(\s+.*$|$)/i],
  'INLINE_CODE': [/(^|^.*\s+)\`(.+)\`(\s+.*$|$)/i],
  'LINK': [/(^|^.*\s+)\[(.+)\]\((.+)\)(\s+.*$|$)/i],
  'SELFLINK': [/(^|^.*\s+)((http|https|ftp)\:\/\/.*)(\s+.*$|$)/i]
})
