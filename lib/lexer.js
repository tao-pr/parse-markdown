...
const TAG = Object.freeze({
  ...
  'SPACEBREAK': '<div class="space"/>@1</div>',
  'LINEBREAK': '<div class="horz-edge">@1</div>',
  'BOLD': '<b>@1</b>',
    ...
    while (remain.length > 0){
      if (!Object.keys(REGEX_INLINE).some(c => {
        return REGEX_INLINE[c].some(r => {
          ...
          if (m){
            if (process.env.isDebug) console.log(`Inline token matched : ${c} => ${JSON.stringify([m[1],m[2],m[3]])}`);

            m.shift()
            let preMatch = this.parseValue(m[0])
            let t = TAG[c]
